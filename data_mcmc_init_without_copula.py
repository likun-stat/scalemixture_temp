...
# -*- coding: utf-8 -*-
"""
Created on Mon Mar  6 17:07:48 2023
...
if __name__ == "__main__":
   ...
   for i in np.arange(n_beta_clusters):
       which_tmp = betaCluster_which[i]
       sigma_beta_logscale0_cluster_proposal.append(np.diag(np.repeat(1, sum(which_tmp))))
       inv_beta_logscale0_cluster_proposal.append((np.diag(np.repeat(1, sum(which_tmp))),np.repeat(1,sum(which_tmp))))       
     
   sigma_beta_logscale1_cluster_proposal=list()
   inv_beta_logscale1_cluster_proposal=list()
   for i in np.arange(n_beta_clusters):
       which_tmp = betaCluster_which[i]
       sigma_beta_logscale1_cluster_proposal.append(np.diag(np.repeat(1, sum(which_tmp))))
       inv_beta_logscale1_cluster_proposal.append((np.diag(np.repeat(1, sum(which_tmp))),np.repeat(1,sum(which_tmp))))       
   
   sigma_beta_logscale2_cluster_proposal=list()
   inv_beta_logscale2_cluster_proposal=list()
   for i in np.arange(n_beta_clusters):
       which_tmp = betaCluster_which[i]
       sigma_beta_logscale2_cluster_proposal.append(np.diag(np.repeat(1, sum(which_tmp))))
       inv_beta_logscale2_cluster_proposal.append((np.diag(np.repeat(1, sum(which_tmp))),np.repeat(1,sum(which_tmp))))       

   sigma_beta_shape_cluster_proposal=list()
   inv_beta_shape_cluster_proposal=list()
   for i in np.arange(n_beta_clusters):
       which_tmp = betaCluster_which[i]
       sigma_beta_shape_cluster_proposal.append(np.diag(np.repeat(1, sum(which_tmp))))
   ...
   if rank == 0:
     ...
     loc1_trace[0,:] = loc1
     loc2_trace = np.empty((n_updates_thinned,n_s)); loc2_trace[:] = np.nan
     loc2_trace[0,:] = loc2
     ...
     logscale1_trace[0,:] = logscale1
     logscale2_trace = np.empty((n_updates_thinned,n_s)); logscale2_trace[:] = np.nan
     logscale2_trace[0,:] = logscale2
     ...
     beta_loc2_trace = np.empty((n_covariates,n_updates_thinned)); beta_loc2_trace[:] = np.nan
     ...
     beta_logscale1_trace[:,0] = beta_logscale1
     beta_logscale2_trace = np.empty((n_covariates,n_updates_thinned)); beta_logscale2_trace[:] = np.nan
     beta_logscale2_trace[:,0] = beta_logscale2
     beta_shape_trace = np.empty((n_covariates,n_updates_thinned)); beta_shape_trace[:] = np.nan
     beta_shape_trace[:,0] = beta_shape
     
     # mu_loc0_trace = np.empty(n_updates_thinned); mu_loc0_trace[:] = np.nan
     # mu_loc0_trace[0] = mu_loc0
     # mu_loc1_trace = np.empty(n_updates_thinned); mu_loc1_trace[:] = np.nan
     # mu_loc1_trace[0] = mu_loc1
     # mu_scale_trace = np.empty(n_updates_thinned); mu_scale_trace[:] = np.nan
     # mu_scale_trace[0] = mu_scale
     # mu_shape_trace = np.empty(n_updates_thinned); mu_shape_trace[:] = np.nan
     # mu_shape_trace[0] = mu_shape
     
     sigma_sbeta_loc0_trace = np.empty(n_updates_thinned); sigma_sbeta_loc0_trace[:] = np.nan
     sigma_sbeta_loc0_trace[0] = sbeta_loc0
     sigma_sbeta_loc1_trace = np.empty(n_updates_thinned); sigma_sbeta_loc1_trace[:] = np.nan
     sigma_sbeta_loc1_trace[0] = sbeta_loc1
     sigma_sbeta_loc2_trace = np.empty(n_updates_thinned); sigma_sbeta_loc2_trace[:] = np.nan
     ...
     sigma_sbeta_logscale1_trace[0] = sbeta_logscale1
     sigma_sbeta_logscale2_trace = np.empty(n_updates_thinned); sigma_sbeta_logscale2_trace[:] = np.nan
     sigma_sbeta_logscale2_trace[0] = sbeta_logscale2
     sigma_sbeta_shape_trace = np.empty(n_updates_thinned); sigma_sbeta_shape_trace[:] = np.nan
     sigma_sbeta_shape_trace[0] = sbeta_shape
